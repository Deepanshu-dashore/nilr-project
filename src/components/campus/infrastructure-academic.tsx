...
export default function InfrastructureAcademic() {
  return (
    ...
    >
      <div className="container-wide pl-5 py-12 md:py-20">

        {/* Section Header */}
        <div className="mb-6 md:mb-8 flex items-center gap-4">
          <BuildingOffice2Icon className="w-6 h-6 md:w-8 md:h-8 text-gray-800" />
        ...
        <p className="text-sm md:text-base md:text-left text-justify text-gray-600 max-w-6xl mb-8 md:mb-12 pr-5">
          At NIRM CVRUK, we prioritize sustainable development and provide modern facilities to support our academic and research programs. Our campus is equipped with state-of-the-art infrastructure, including classrooms, labs, and research facilities, to ensure a conducive learning environment.
        </p>

        {/* Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 border-t border-l border-gray-200 shadow-premium">
          ...
          {facilityCards.map((card, index) => {
            ...
            return (
              <div
                key={index}
                className="bg-white border-r border-b border-gray-200 p-5 md:p-6 flex flex-col group"
              >
                {card.img && (
                  <div className="mb-5 overflow-hidden">
                    <img
                      src={card.img}
                      alt={card.title}
                      className="w-full h-44 md:h-48 object-cover transform group-hover:scale-105 transition-transform duration-700"
                    />
                  </div>
                )}
                ...
                {/* List */}
                <ul className="space-y-2 text-xs md:text-sm text-gray-600 flex-1">
                  {card.items.map((item, i) => (
                    <li key={i} className="flex gap-2 items-start">
                      <div className="w-5 h-5 md:w-6 md:h-6 text-green-600 mt-0.5 shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 md:w-8 md:h-8 text-primary -mt-1 md:-mt-2 shrink-0" viewBox="0 0 256 256">
                          <g fill="currentColor">
                ...
                {card.badge && (
                  <span className="mt-4 text-[10px] md:text-xs font-semibold text-primary uppercase tracking-widest">
                    {card.badge} Enabled
                  </span>
                )}
              </div>
